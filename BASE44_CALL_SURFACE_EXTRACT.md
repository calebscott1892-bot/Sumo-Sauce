# Base44 Call Surface Extract (generated)

Generated by scanning `src/` for call-sites. Evidence links are code file+line.

## base44.entities Entity+Method calls
- Unique entities (by method calls): 18
- Unique (entity, method) pairs: 45
...
## base44.integrations accesses
Note: repo shows export-surface accesses; no other call-sites detected by pattern scan.
...
## Missing helper module imports (.+@/api/functions/*)
- Unique modules: 11
...
## Polling/timer sites
All `refetchInterval`, `setInterval`, `setTimeout` matches (not all are “realtime”).

...
async function main() {
  let files;
  try {
    files = await listFiles(PAGES_DIR);
  } catch (e) {
    console.error(`Failed to read pages dir: ${PAGES_DIR}`);
  ...
  if (violations.length) {
    console.error('Direct network calls are not allowed in src/pages/**');
  ...
  console.log('OK: no direct fetch/axios usage found in src/pages/**');
}

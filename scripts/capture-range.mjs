...
function runCaptureOne(bashoId) {
  const result = spawnSync('node', ['scripts/capture-basho.mjs', '--basho', bashoId], {
    encoding: 'utf8',
  ...
  if (result.status !== 0) {
    return {
      ...
      error: String(result.stderr || result.stdout || 'capture failed').trim(),
    };
  }
...
async function main() {
  ...
  if (!from || !to) {
    throw new Error('Usage: npm run capture:range -- --from YYYYMM --to YYYYMM');
  }
  ...
  for (const bashoId of ids) {
    const row = runCaptureOne(bashoId);
    if (row.ok) ok += 1;
